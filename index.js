...
const averagePage = fs.readFileSync(`${__dirname}/templates/averagePage.html`,`utf-8`);
const averageTable = fs.readFileSync(`${__dirname}/templates/averageTable.html`,`utf-8`);
...
const server = http.createServer((req, res) => {
  ...
  switch (pathname) {
    ...
    ///////////////////////////////////////////////////////////////////////////// +
    case "/searchByClass":
      const outputClass = main
        .replace(`{%STUDENT_CARDS%}`, students
          .filter(student => student.class.includes(query.class))
          .map(student => replaceTemplate(card, student))
          .join(''))
        .replace(`{%CLASS%}`, sortAndFilterClass(students)
          .map(studentClass => replaceTemplate(dropdawnItem, studentClass))
          .join(''));
  ...
  case "/average":
    ...
    const outputAverage = averagePage.replace(`{%AVERAGE_TABLE%}`, tableRow);
